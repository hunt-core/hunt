...
- `jobs_history` table — generated by `hunt queue:table`, records every completed and permanently failed job with `job_class`, `queue`, `duration_ms`, `finished_at`, and `status` (`completed` / `failed`).
- Queue worker (`hunt queue:work`) now writes to `jobs_history` on job completion and final failure. The table is optional — if absent, the worker silently skips recording.
- Queue admin dashboard gains a **Throughput** section: 24-hour bar chart (per-hour buckets, green=completed / red=failed) driven by `jobs_history`.
- Queue admin dashboard gains a **Queue breakdown** table: per-queue pending count, jobs processed in the last hour, and jobs failed in the last hour.
- Failed job **View** modal: clicking View on any failed job opens a `<dialog>` showing the full exception text and pretty-printed payload JSON, with a Retry button in the modal footer.
- `_format_payload()` helper in queue controller — unwraps the signed envelope and pretty-prints the inner job body as JSON for display in the modal.
